$(document).ready(function(){
    ...
    fetch(`https://api.openweathermap.org/data/2.5/weather?q=${city}&units=metric&appid=${apiKey}`)
      ...
      .then(data => {
        ...
        if (!searchHistory.includes(city)) {
          ...
          createSearchHistory(searchHistory);
        }
  ...
  function createForecastCards(data) {
    ...
    for (let i = 0; i < data.list.length; i += 8) {
      ...
      const temperature = document.createElement('p');
      temperature.innerHTML = 'Temperature: ${data.list[i].main.temp}°C';
    }
  }
  ...
  function createSearchHistory(searchTerm) {
    var searchDate = new Date().toLocaleString();
    var listItem = $("<li>").text(searchTerm + " - " + searchDate);
    searchHistory.unshift(listItem);
  }
  });
